...
public class AddressBookDAOImplementation implements AddressBookDAO{
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update AddressBook set name='"+person.getName()+"',mob='"+person.getMob()+"',mail='"+person.getEmail()+"'+,city='"+person.getCity()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    public void getSelectedName(DetailViewPanel detailPanel,String selectedName)
    {
        ...
        try {
            ...
            while(rs.next()){
             ...
             detailPanel.seteMail(rs.getString("mail"));
    ...
    @Override
    public void searchPerson(NameListPanel namePanel, String str) {
 DefaultListModel<String> dlm = new DefaultListModel<>();
            try {
         conn = DBConnection.getInstance().getConnect();
            Statement stmt;
                 stmt = conn.createStatement();
                 str = str+'%';
            String qry = "select * from AddressBook where name like '" + str + "' order by name asc";
                 try (ResultSet rs = stmt.executeQuery(qry)) {
                     while (rs.next()){
                         String name = rs.getString(1);
                         dlm.addElement(name);
